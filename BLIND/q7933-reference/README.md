...
A manual diagnostics executable compares the exact coordinate baseline, the 256-digit global reduction, and exact coordinate cleanup at `d=16`, `32`, and `64` with `q=7933`:
...
2. Compare global reduction versus the exact coordinate baseline through `d=64` and require exact postconditions at every stage.
3. Only if the global reducer materially reduces the cleanup work, open a separate `d=128` diagnostic gate, followed by `d=256`.
