...
int main() {
    try {
        // This is deliberately the same sparse deterministic anchor used by
        // all smaller scaling gates. d=512 is the actual BLNS23 ring-size
        // machinery/precision checkpoint, not yet a claim about the eventual
        // TrapGen coefficient distribution or Gaussian-sampling quality.
        ...
        if (!solved) {
            throw std::runtime_error(
                "deterministic d=512 candidate was rejected by NTRUSolve");
        }
        require_relation(f, g, *solved, q, "d=512 solver");

        const BigInt raw_norm =
            NTRUBasisReducer::squared_norm(solved->F, solved->G);

        // Keep exactly the same eight-round cap as the d=128 and d=256 gates.
        // State-changing updates remain protected by exact relation and norm
        // checks inside the global reducer.
        ...
        require_relation(f, g, global, q, "d=512 global reducer");

        const BigInt global_norm =
            NTRUBasisReducer::squared_norm(global.F, global.G);
        if (global_norm > raw_norm) {
            throw std::logic_error(
                "d=512 global reducer increased exact squared norm");
        }

        // Exact coordinate cleanup is retained only as an audit backstop. The
        // intentionally expensive direct coordinate baseline is not run from
        // the raw d=512 solution.
        ...
        require_relation(f, g, cleaned, q, "d=512 exact cleanup");

        const BigInt cleaned_norm =
            NTRUBasisReducer::squared_norm(cleaned.F, cleaned.G);
        if (cleaned_norm > global_norm) {
            throw std::logic_error(
                "d=512 exact cleanup increased global squared norm");
        }

        std::cout << std::fixed << std::setprecision(3);
        std::cout << "BLNS7933 explicit d=512 gate, q=" << kQ << '\n';
        std::cout << "  solver:                      "
        ...
        std::cout << "BLNS7933 d=512 diagnostic: OK\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "BLNS7933 d=512 diagnostic: FAIL: " << e.what() << '\n';
        return 1;
    }
}
