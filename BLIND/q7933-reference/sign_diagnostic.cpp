// Full pipeline diagnostic: TrapGen -> build the signing tree -> Sign ->
// Verify, at the REAL modulus q=7933 and a configurable ring dimension
// (see kDegree below). Deliberately NOT registered with CTest - the LDL
// tree construction's div() calls cost O(degree^3) at 256-digit
// precision (same order of cost already measured for the quality-bound
// module's single such call), so this genuinely takes minutes at larger
// dimensions, matching every other scaling diagnostic in this directory.
...
// Change this to scale up (32 -> 64 -> 128 -> 512), matching how every
// other diagnostic in this directory was staged. Kept at 32 by default so
// this remains practical to re-run during development; the real d=512
// case is run and its result recorded manually (see README.md), not as
// this file's default target, matching scaling_512_diagnostic.cpp's own
// precedent.
