...
constexpr std::size_t kDegree = 256U;
...
int main() {
    try {
        ...
        // the smaller scaling gates.  d=256 is a machinery/precision gate,
        // not yet a claim about the eventual TrapGen coefficient distribution.
        ...
        if (!solved) {
            throw std::runtime_error(
                "deterministic d=256 candidate was rejected by NTRUSolve");
        }
        require_relation(f, g, *solved, q, "d=256 solver");

        const BigInt raw_norm =
            NTRUBasisReducer::squared_norm(solved->F, solved->G);

        // The global reducer is capped at eight rounds just like the d=128
        // gate.  Exact relation and norm checks guard every accepted round.
        ...
        require_relation(f, g, global, q, "d=256 global reducer");

        const BigInt global_norm =
            NTRUBasisReducer::squared_norm(global.F, global.G);
        if (global_norm > raw_norm) {
            throw std::logic_error(
                "d=256 global reducer increased exact squared norm");
        }

        // Exact coordinate cleanup remains an audit backstop.  We do not run
        // the expensive direct coordinate baseline from the raw solution.
        ...
        require_relation(f, g, cleaned, q, "d=256 exact cleanup");

        const BigInt cleaned_norm =
            NTRUBasisReducer::squared_norm(cleaned.F, cleaned.G);
        if (cleaned_norm > global_norm) {
            throw std::logic_error(
                "d=256 exact cleanup increased global squared norm");
        }

        std::cout << std::fixed << std::setprecision(3);
        std::cout << "BLNS7933 explicit d=256 gate, q=" << kQ << '\n';
        std::cout << "  solver:                      "
        ...
        std::cout << "BLNS7933 d=256 diagnostic: OK\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "BLNS7933 d=256 diagnostic: FAIL: " << e.what() << '\n';
        return 1;
    }
}
